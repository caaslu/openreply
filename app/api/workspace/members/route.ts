...
export async function POST(request: NextRequest) {
  ...
  if (!canManageWorkspace(context.role)) {
    return NextResponse.json(
      { success: false, error: "Apenas proprietários e administradores podem convidar membros" },
      { status: 403 }
    );
  }
...
export async function PATCH(request: NextRequest) {
  ...
  if (!canManageWorkspace(context.role)) {
    return NextResponse.json(
      { success: false, error: "Apenas proprietários e administradores podem atualizar funções" },
      { status: 403 }
    );
  }
  ...
  if (!member || member.role === "OWNER") {
    return NextResponse.json(
      { success: false, error: "O membro não pode ser atualizado" },
      { status: 400 }
    );
  }
...
export async function DELETE(request: NextRequest) {
  ...
  if (!canManageWorkspace(context.role)) {
    return NextResponse.json(
      { success: false, error: "Apenas proprietários e administradores podem remover membros" },
      { status: 403 }
    );
  }
  ...
  if (parsed.data.memberId) {
    ...
    if (!member || member.role === "OWNER" || member.userId === context.userId) {
      return NextResponse.json(
        { success: false, error: "O membro não pode ser removido" },
        { status: 400 }
      );
    }
