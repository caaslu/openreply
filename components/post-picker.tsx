...
export default function PostPicker({
    ...
    fetch(`/api/instagram/posts${params.size ? `?${params}` : ""}`)
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          setError(data.error ?? "Falha ao carregar publicações");
        }
      })
      .catch(() => {
        if (!cancelled && !cached.data) setError("Falha ao carregar publicações");
      })
      .finally(() => {
  ...
  if (posts.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-muted">Nenhuma publicação encontrada</p>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <input
          ...
          placeholder="Busque suas publicações pela legenda…"
      ...
      {visible.length === 0 ? (
        <p className="py-6 text-center text-sm text-muted">
          Nenhuma publicação corresponde a &ldquo;{query}&rdquo;
        </p>
      ) : (
        <>
          ...
          <button
            ...
            title={isUsed ? `Já utilizada por "${usedByName}"` : undefined}
            className={`
          ...
          >
            {thumb ? (
              <img
                src={thumb}
                alt={post.caption?.slice(0, 50) ?? "Publicação do Instagram"}
                loading="lazy"
                decoding="async"
