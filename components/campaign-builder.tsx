...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (triggerScope === "specific" && !postId)
      return setError("Escolha uma publicação ou reel para disparar a campanha.");
    ...
    if (!dmMessage.trim()) return setError("Adicione o DM com o link.");
    if (openingDmEnabled && (!openingDmMessage.trim() || !openingDmButtonLabel.trim()))
      return setError("O DM de abertura precisa de uma mensagem e um rótulo de botão.");
  ...
  return (
    <div className="space-y-6">
      {importQueue && (
        <div className="rounded border border-accent/30 bg-accent/5 px-4 py-3 text-sm">
          ...
          <span className="text-muted">
            Os campos vêm preenchidos do seu CSV. Escolha o reel, edite o que
            quiser e salve para carregar o próximo — ou Pular se não quiser este.
          </span>
        </div>
      )}
      ...
      <div className="space-y-8 min-w-0">
        ...
        <Section title="E este comentário tiver">
          <Radio
            checked={matchMode === "specific"}
            onSelect={() => setMatchMode("specific")}
          >
            uma palavra ou palavras específicas
          </Radio>
          {matchMode === "specific" && (
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              também responder quando alguém mandar DM{" "}
              {matchMode === "any" ? "com qualquer coisa" : "com essas palavras"}
            </span>
            <Toggle
              on={dmTriggerEnabled}
        ...
        <Section title="Eles vão receber">
          ...
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-foreground">
                uma exigência de seguir primeiro
              </span>
              <Toggle
                on={requireFollow}
            ...
            {requireFollow && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="um favor rápido antes de eu enviar seu link. eu não ganho nada com isso, é de graça. se quiser me apoiar, é só não deixar de seguir depois e dar estrela no repositório do github se ajudar. toque no botão quando estiver seguindo que eu envio"
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                />
                <p className="text-xs text-muted">
                  Enviamos o link só depois que eles tocarem no botão e o Instagram
                  confirmar que está seguindo. Se não der para verificar, enviamos mesmo
                  assim.
                </p>
              </div>
            )}
        ...
        <Section title="E então, eles vão receber">
          <div className="rounded-lg border border-border p-3 space-y-2">
            <span className="text-sm text-foreground">um DM com um link</span>
            <textarea
              value={dmMessage}
            ...
            {linkOpen ? (
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="Rótulo do botão (ex.: Abrir link)"
                  maxLength={20}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                />
                {secondLinkOpen ? (
                  <div className="space-y-2 border-t border-border pt-2">
                    ...
                    <input
                      ...
                      placeholder="Rótulo do segundo botão"
                      maxLength={20}
                      className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                    />
                  </div>
          ...
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-foreground">
                uma mensagem de acompanhamento de agradecimento
              </span>
              <Toggle
                on={followUpEnabled}
            ...
            {followUpEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Ah, só queria agradecer por me seguir, valeu demais o apoio 🙌"
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                <p className="text-xs text-muted">
                  {followUpDelayMinutes > 0
                    ? `Enviada ${followUpDelayMinutes} min depois que eles tocarem no link.`
                    : "Enviada logo depois que eles tocarem no link."}
                  {" {username}"} personaliza. Máximo de 24 horas, para ficar na
                  janela de mensagens do Instagram.
                </p>
              </div>
            )}
          </div>
