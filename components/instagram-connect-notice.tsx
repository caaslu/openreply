...
const MESSAGES: Record<string, { tone: Tone; title: string; detail: string }> = {
  ...
  invalid: {
    ...
    detail:
      "O link de login estava ausente ou tem mais de 10 minutos. Clique em Conectar Instagram para tentar de novo.",
  },
  forbidden: {
    tone: "error",
    title: "Sem permissão",
    detail:
      "Apenas proprietários e administradores do workspace podem conectar uma conta do Instagram.",
  },
  already_connected: {
    tone: "warning",
    title: "Conta já conectada",
    detail:
      "Essa conta do Instagram já está conectada a outro workspace. Desconecte-a lá primeiro, ou conecte outra conta.",
  },
};
  ...
  if (status === "failed") {
    ...
    return (
      <Notice tone="error" title="Falha na conexão com o Instagram">
        <p>
          O Instagram aceitou o login, mas a conexão não pôde ser concluída. Em
          geral isso é um redirect URI incompatível ou um app sem as permissões
          necessárias.
        </p>
        {reason && (
